...
async function loadAllConfig() {
    ...
    if (error) {
        toast("Gagal memuat konfigurasi: " + error.message, "error");
        return;
    }

    configData = {};
    (data || []).forEach(row => {
        configData[row.key] = row;
    });
...
async function saveSection(section) {
    ...
    if (section === "server") {
        ...
        updates = {
            ...
            status_show_latency: document.getElementById("toggle-status_show_latency").classList.contains("on")
                ? "true"
                : "false",
            status_show_playerlist: document.getElementById("toggle-status_show_playerlist").classList.contains("on")
                ? "true"
                : "false",
            status_pause_hidden: document.getElementById("toggle-status_pause_hidden").classList.contains("on")
                ? "true"
                : "false",
        };
...
async function fetchLiveStatus() {
    ...
    try {
        const res = await fetch(`https://api.mcsrvstat.us/bedrock/3/${host}:${port || 19214}`, {
