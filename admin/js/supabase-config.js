...
function switchAuthTab(tab) {
  const track = document.getElementById('auth-slide-track');
  const tabLogin    = document.getElementById('tab-pill-login');
  const tabRegister = document.getElementById('tab-pill-register');
  if (!track) return;
...
function togglePw(inputId, btn) {
  ...
  if (btn) btn.style.color = show ? 'var(--accent)' : 'var(--text-faint)';
}
window.togglePw = togglePw;

// ==================== MAINTENANCE TOGGLE PREVIEW ====================
function updateMaintPill() {
  const on   = document.getElementById('toggle-maintenance_active')?.classList.contains('on');
...
window.updateMaintPill = updateMaintPill;
