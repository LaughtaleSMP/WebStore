...
function buildModal(item) {
  ...
  const origBadge = (item.originalPrice && item.originalPrice > item.price)
    ? `<span style="font-size:0.65rem;color:var(--s-muted);text-decoration:line-through;margin-left:6px;display:inline-block; transform:translateY(-0.25em); line-height:1">${fmtPlain(item.originalPrice)}</span>` : '';
