    ...
    if (typeof window.shopBuildCard === 'function') {
      ...
      gridEl.innerHTML = items.map(item => {
        ...
        return `<div class="shop-card${sold ? ' shop-sold-out' : ''}" data-category="${item.category}">
          ...
          ${(function(){ var _i = item.images && item.images[0]; return typeof _i === 'string' ? _i : (_i && _i.url ? _i.url : ''); })()
            ? `<img src="${(function(){ var _i = item.images && item.images[0]; return typeof _i === 'string' ? _i : (_i && _i.url ? _i.url : ''); })()}" alt="${item.name}" style="width:100%;aspect-ratio:4/3;object-fit:cover;display:block;background:#111827">`
